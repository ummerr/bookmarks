...
function RandomPageInner() {
  ...
  // On initial load: restore from ?id= if present, else fetch random
  useEffect(() => {
          ...
          <div className="flex items-center gap-2">
            {(['image', 'video'] as const).map((g) => (
              <button
                key={g}
                onClick={() => {
                  const next = group === g ? null : g
                  setGroup(next)
                  fetchRandom(next, multiShot)
                }}
            ...
            <button
              onClick={() => {
                const next = !multiShot
                setMultiShot(next)
                fetchRandom(group, next)
              }}
